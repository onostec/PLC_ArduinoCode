...
void loop() {
  ...
  T1.TOF(T1.EN, T1.RES, T1.PT = 5, T1.BASE = "Seconds", T1.DN, T1.TT, T1.ET);

    if (S1) {//Enable Timer
      T1.EN = true;
      lcd.setCursor(0,0);              //display ON at column 0, row 0 of the LCD
      lcd.print("TON: ON ");
    } else {
      T1.EN = false;
      lcd.setCursor(0,0);              //display ON at column 0, row 0 of the LCD
    ...
    if (T1.DN){ //Timer Done
      R1 = true;
      lcd.setCursor(9,0);              //display ON at column 9, row 0 of the LCD
      lcd.print("DN: ON ");
    }
    else {
      R1 = false;
    ...
    lcd.setCursor(0,1);              //display ON at column 0, row 1 of the LCD
    lcd.print("ELASPED TIME: ");
    lcd.setCursor(14,1);              //display ON at column 14, row 1 of the LCD
    lcd.print(T1.ET);
